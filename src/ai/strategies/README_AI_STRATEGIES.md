...
```typescript
// Клонирование стратегии от одного корабля к другому
AIExampleUsage.cloneStrategy(scene, sourceShipId, targetShipId);
```
...
class TorpedoEvasionStrategy {
    ...
    performEvasiveManeuver(owner, torpedo) {
        ...
        const evasionX = owner.x + Math.cos(evasionAngle) * evasionDistance;
        const evasionY = owner.y + Math.sin(evasionAngle) * evasionDistance;
